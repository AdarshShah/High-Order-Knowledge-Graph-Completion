...
@torch.no_grad()
def _generate_nx_graph(path = 'hyperedges.txt'):
    '''
    Returns 
...
@torch.no_grad()
def get_dgl_graph(skip_cache=False):
    '''
    Returns 
    Directed dgl graph, 
    ...
    nx_graph = None
    graph = None
    if os.path.exists(os.path.join(dirname,'cache/nx_graph.pkl')) and not skip_cache:
        nx_graph = pickle.load(open(os.path.join(dirname,'cache/nx_graph.pkl'),'rb'))
    if os.path.exists(os.path.join(dirname,'cache/dgl_graph.pkl')) and not skip_cache:
        graph = pickle.load(open(os.path.join(dirname,'cache/dgl_graph.pkl'),'rb'))
    if nx_graph is None:
        nx_graph = _generate_nx_graph()
        pickle.dump(nx_graph,open(os.path.join(dirname,'cache/nx_graph.pkl'),'wb'))
    else:
    ...
    if graph is None:
        ...
        pickle.dump(graph,open(os.path.join(dirname,'cache/dgl_graph.pkl'),'wb'))
    else:
